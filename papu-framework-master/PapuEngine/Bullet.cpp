...
void Bullet::draw1(SpriteBacth& spriteBatch) {
	Color color;
	color.r = 255;
	color.g = 255;
	color.b = 255;
	...
	glm::vec4 posAnSize = glm::vec4(_position.x, _position.y, 30, 30);
	spriteBatch.draw(posAnSize, uv, texture.id, 0.0f, color);
}void Bullet::draw2(SpriteBacth& spriteBatch) {
	Color color;
	color.r = 255;
	color.g = 255;
	color.b = 255;
	color.a = 255;
	glm::vec4 uv(0.0f, 0.0f, 1.0f, 1.0f);
	static GLTexture texture = ResourceManager::getTexture("Textures/glass.png");
	glm::vec4 posAnSize = glm::vec4(_position.x, _position.y, 30, 30);
	spriteBatch.draw(posAnSize, uv, texture.id, 0.0f, color);
}void Bullet::draw3(SpriteBacth& spriteBatch) {
	Color color;
	color.r = 255;
	color.g = 255;
	color.b = 255;
